...
## [0904-0.1.2-alpha.3] - 2026-09-04

### 新增
- **verylook_upload_image 工具**：图片（base64/本地路径）上传 Cloudflare R2（r2-very-im），返回公网 URL（r2.very.im），供出图技能转 URL 渲染与图生图参考
- **设置面板改版**：去掉重复小标题，「功能检测」整卡可点（hover 品牌色反馈），开关/检测/模型按钮三块平铺

### 修复
- **引用迭代**：引用标记提示词三处对齐（verylook-skill + umfun/agnes 技能），模型必须把引用 URL 作为参考图传给出图 API，禁止无视引用文生图
- 引用格式改为明文 URL（图片：URL），旧标记格式保留兼容
...
## [0903-0.1.2-alpha.3] - 2026-09-03

### 新增
- **功能检测弹窗**：检测项从 6→11 项（新增 PDF、ZIP、会话引用、渲染增强、上传通道检测），按钮改名「功能检测」，弹窗展示全部能力状态
- **设置面板 UI 简化**：去掉「功能开关」「功能检测」重复小标题，三项内容平铺为完整卡片
- **PDF 解析能力**：使用 pdfjs-dist 解析 PDF 文档，支持纯文本提取与分页
- **ZIP 解压能力**：使用 adm-zip 解压压缩包，查看内部文件结构与内容
- **会话引用**：支持 dsh-session:// 引用格式，跨会话传递上下文
- **渲染增强**：出图/出视频结果自动渲染为缩略图卡片（带视频播放按钮）
- **文件上传通道**：非图片/视频文件（PDF/Office/ZIP/PSD 等）走独立上传通道
- **引用系统**：缩略图引用按钮 → 输入框上方引用栏 → 发送合并引用标记 → 消息渲染引用卡片

### 优化
- **UI 规范化**：全线对齐 DSH-UI-SPEC 规范，弹窗遮罩/关闭按钮/卡片/开关全部使用主题变量
- **拖放悬停反馈**：拖拽文件到聊天框时显示 DropOverlay 上传标识
- **视频缩略图播放图标**：输入框 + 已发送消息中的视频 tile 均显示播放按钮 overlay
- **非图片文件图标**：优化文件类型图标大小与位置，对齐原生 64px tile

### 修复
- **verypass 保存密钥 Bug**：`seen.add(variable)` 提前标记导致所有单值/多值密钥被静默跳过
- **视频识别路径**：`source` 解析先用 `existsSync` 检查 `.uploads/` 路径
- **渲染增强器死代码**：`render-enhancer.ts` 从未被引用，重写后接入 index.ts

### 变更
- 版本号：`0901-0.1.2-alpha.3` → `0903-0.1.2-alpha.3`
- 删除 makemake 插件（根据主任指示全删）
