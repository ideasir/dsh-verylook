...
### 引用图片/视频（图生图·迭代，重要）
- 用户消息里的独立行「图片：<URL>」或「视频：<URL>」是用户通过引用按钮选中的媒体（明文公网地址，无需解析任何协议）。
- 当用户说「基于这张图改 / 迭代 / 继续 / 换风格 / 换背景 / 图生图 / 合成」等时，**必须**把该 URL 作为参考图传给图像生成 API（如 UmFun 的顶层 image 字段、Agnes 的 extra_body.image），**禁止无视引用直接文生图**。
- 如果需要先理解图片内容再改，可调 verylook_see 分析该 URL；但参考图 URL 本身必须传给生成 API。
- 出图后渲染：最终回复用 ![标题](结果URL) 展示，不用再引用旧图。
`
